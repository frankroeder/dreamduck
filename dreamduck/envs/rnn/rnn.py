...
def default_hps():
    return HyperParams(
        num_steps=2000,  # train model for 2000 steps.
        max_seq_len=1000,  # train on sequences of 100
        # width of our data (64 + 2 actions)
        input_seq_width=66,
        output_seq_width=64,    # width of our data
...
hps_sample = hps_model._replace(
    batch_size=1, max_seq_len=1, use_recurrent_dropout=0, is_training=0)
...
class MDNRNN():
    ...
    def build_model(self, hps):
        ...
        self.input_x = tf.placeholder(dtype=tf.float32, shape=[
                                      self.hps.batch_size, self.hps.max_seq_len, INWIDTH])
        self.output_x = tf.placeholder(dtype=tf.float32, shape=[
                                       self.hps.batch_size, self.hps.max_seq_len, OUTWIDTH])
        ...
        if self.hps.is_training == 1:
            ...
            capped_gvs = [(tf.clip_by_value(grad, -self.hps.grad_clip,
                                            self.hps.grad_clip), var) for grad, var in gvs]
...
def sample_sequence(sess, s_model, hps, init_z, actions, temperature=1.0, seq_len=1000):
    ...
    for i in range(seq_len):
        input_x = np.concatenate(
            (prev_x, actions[i].reshape((1, 1, 2))), axis=2)
        feed = {s_model.input_x: input_x, s_model.initial_state: prev_state}
        [logmix, mean, logstd, next_state] = sess.run([
            s_model.out_logmix, s_model.out_mean, s_model.out_logstd, s_model.final_state
        ], feed)
...
def rnn_next_state(rnn, z, a, prev_state):
    input_x = np.concatenate(
        (z.reshape((1, 1, 64)), a.reshape((1, 1, 2))), axis=2)
