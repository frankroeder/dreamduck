...
def default_hps():
    return HyperParams(
        num_steps=5000,
        max_seq_len=500,
        input_seq_width=66,  # width of our data (64 + 2 actions)
...
class MDNRNN():
    ...
    def build_model(self, hps):
        ...
        self.input_x = tf.placeholder(dtype=tf.float32,
                                      shape=[self.hps.batch_size,
                                             self.hps.max_seq_len, INWIDTH])
        self.output_x = tf.placeholder(dtype=tf.float32,
                                       shape=[self.hps.batch_size,
                                              self.hps.max_seq_len, OUTWIDTH])
    ...
    def save_json(self, jsonfile='rnn.json'):
        ...
        with open(jsonfile, 'wt') as outfile:
            json.dump(qparams, outfile, sort_keys=True, indent=0,
                      separators=(',', ': '))
...
def sample_sequence(sess, s_model, hps, init_z, actions, temperature=1.0,
                    seq_len=1000):
    ...
    for i in range(seq_len):
        input_x = np.concatenate((prev_x, actions[i].reshape((1, 1, 2))),
                                 axis=2)
        feed = {s_model.input_x: input_x,
                s_model.initial_state: prev_state}

        [logmix, mean, logstd, next_state] = sess.run([s_model.out_logmix,
                                                       s_model.out_mean,
                                                       s_model.out_logstd,
                                                       s_model.final_state]
                                                      , feed)
...
def rnn_next_state(rnn, z, a, prev_state):
    input_x = np.concatenate((z.reshape((1, 1, 64)),
                              a.reshape((1, 1, 2))), axis=2)
...
def rnn_output_size(mode):
    if mode == MODE_ZCH:
        return (64+model_rnn_size+model_rnn_size)
    if (mode == MODE_ZC) or (mode == MODE_ZH):
        return (64+model_rnn_size)
