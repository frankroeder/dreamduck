...
hps_sample = hps_model._replace(batch_size=1, max_seq_len=1,
                                use_recurrent_dropout=0, is_training=0)
...
def create_batches(all_data, batch_size=100, seq_length=500):
    ...
    data_logvar = np.split(data_logvar.reshape( batch_size, -1, 64),
                           num_batches, 1)
    data_action = np.split(data_action.reshape(batch_size, -1, 2),
                           num_batches, 1)
...
for local_step in range(hps.num_steps):
    ...
    (train_cost, state, train_step, _) = rnn.sess.run([
        rnn.cost, rnn.final_state, rnn.global_step, rnn.train_op
    ], feed)
