...
def default_hps():
    return HyperParams(
        num_steps=4000,
        # train on sequences of 1000 (so 999 + teacher forcing shift)
        max_seq_len=999,
        # width of our data (32 + 3 actions)
        input_seq_width=35,
...
for local_step in range(hps.num_steps):
    ...
    print(output_log)
    if local_step % 500 == 0:
        rnn.save_json(os.path.join(
            model_save_path, "rnn_{}.json".format(local_step)))
